...
export type SectionHeaderIconVariant = keyof typeof ICON_VARIANT_CLASS;

export type SectionHeaderProps = {
  variant?: SectionHeaderIconVariant;
  icon: ReactNode;
  title: string;
  subtitle?: string;
  trailing?: ReactNode;
...
const TITLE_CLASS = 'truncate text-xs font-semibold tracking-tight text-text-primary sm:text-sm';
const SUBTITLE_CLASS = 'mt-0.5 text-[11px] leading-snug text-text-muted sm:text-xs';
  ...
  if (subtitle) {
    return (
      <header className={cn('flex shrink-0 items-start gap-2', className)}>
        <span className={cn(ICON_BOX, variantClass)} aria-hidden>
          {icon}
        </span>
