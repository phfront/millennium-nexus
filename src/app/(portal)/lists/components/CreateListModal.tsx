...
export function CreateListModal({ onClose, onSuccess }: CreateListModalProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await createList({ name: name.trim(), icon, color: color ?? undefined, household_id: householdId });
        onSuccess?.();
        onClose();
      } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl bg-surface-2 border border-border shadow-2xl p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-text-primary">Nova lista</h2>
          <button
            onClick={onClose}
