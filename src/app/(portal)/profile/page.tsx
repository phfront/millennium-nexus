...
import {
  normalizeBrandHex,
  RASTER_BRAND_PRIMARY_HEX,
  RASTER_BRAND_SECONDARY_HEX,
...
const TIMEZONE_OPTIONS = [
  {
    ...
    label: "Brasil — Brasília / São Paulo (UTC-3)",
  },
...
export default function ProfilePage() {
  ...
  const [brandPrimary, setBrandPrimary] = useState<string | null>(
    profile?.brand_primary_hex ?? null
  );
  const [brandSecondary, setBrandSecondary] = useState<string | null>(
    profile?.brand_secondary_hex ?? null
  );
  const [brandPrimaryText, setBrandPrimaryText] = useState(
    profile?.brand_primary_hex ?? ""
  );
  const [brandSecondaryText, setBrandSecondaryText] = useState(
    profile?.brand_secondary_hex ?? ""
  );
  ...
  async function saveBrandColors() {
    if (!user) return;
    const p =
      brandPrimaryText.trim() === ""
        ? null
        : normalizeBrandHex(brandPrimaryText);
    const s =
      brandSecondaryText.trim() === ""
        ? null
        : normalizeBrandHex(brandSecondaryText);
    if (brandPrimaryText.trim() !== "" && p === null) {
      toast.error(
        "Hex inválido",
        "Use formato #RRGGBB na cor primária ou deixe vazio."
      );
      return;
    }
    if (brandSecondaryText.trim() !== "" && s === null) {
      toast.error(
        "Hex inválido",
        "Use formato #RRGGBB na cor secundária ou deixe vazio."
      );
      return;
    }
      ...
      <Card>
        ...
        <Card.Body className="space-y-5">
          <p className="text-xs text-text-muted">
            Personalize a cor primária e secundária da interface. Os valores são
            guardados na sua conta e espelhados num cookie para carregar mais
            depressa.
          </p>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  value={brandPrimary ?? RASTER_BRAND_PRIMARY_HEX}
                  onChange={(e) => {
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  value={brandSecondary ?? RASTER_BRAND_SECONDARY_HEX}
                  onChange={(e) => {
                    const v = normalizeBrandHex(e.target.value);
                    if (v) {
                      setBrandSecondary(v);
                ...
                <Input
                  ...
                  placeholder="#beac4e ou vazio = padrão"
                  className="flex-1 min-w-32"
                />
              </div>
            </div>
